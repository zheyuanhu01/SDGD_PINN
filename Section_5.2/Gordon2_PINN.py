...
class Gordon(FBSNN):
    def __init__(self, Xi, T, M, N, D, layers, device, method_PINN, batch_size_PINN):
        super().__init__(Xi, T, M, N, D, layers, device, method_PINN, batch_size_PINN, "Gordon")
        self.model = MLP(layers, D).to(device)
        if D == 10:
            self.u_exact = 0.3229470
        elif D == 100:
            self.u_exact = 0.0528368
        elif D == 1000:
            self.u_exact = 0.0055896
        elif D == 10000:
            self.u_exact = 0.0005621
        self.u_exact *= self.D
    
    def phi_tf(self, t, X, Y, Z): # M x 1, M x D, M x 1, M x D
        return -self.D * torch.sin(Y / self.D) # M x 1
    
    def g_tf(self, X): # M x D
        #return torch.sum(X, 1, keepdims=True) # M x 1
        return 5 / (10 + 2 * torch.sum(X**2, 1, keepdims=True)) * self.D # M x 1
    ...
    def residual(self, t, X):
        ...
        for i in range(self.D):
            ...
            u_xx += d2f_dxidxi
        u = u.reshape(-1)
        residual_pred = u_t.reshape(-1) + (u_xx + self.D * torch.sin(u / self.D))
    ...
    def residual_1(self, t, X):
        ...
        u_x = torch.autograd.grad(u.sum(), X, create_graph=True)[0]
        u = u.reshape(-1)
        residual_pred = (u_t.reshape(-1) + self.D * torch.sin(u / self.D)).detach()
        gradient_j = u_t.reshape(-1) + self.D * torch.sin(u / self.D)
...
if __name__ == "__main__":
    ...
    # Training
    model = Gordon(Xi, T, M, N, D, layers, device, args.method_PINN, args.batch_size_PINN)
    ...
    df.to_excel(
        "Gordon2_FBSNN_PINN_"+str(args.dim)+"_"+str(args.SEED)+".xlsx",
        index=False
    )