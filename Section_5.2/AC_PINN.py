...
class AllenCahn(FBSNN):
    def __init__(self, Xi, T, M, N, D, layers, device, method_PINN, batch_size_PINN):
        super().__init__(Xi, T, M, N, D, layers, device, method_PINN, batch_size_PINN, "AC")
        self.model = MLP(layers, D).to(device)
        if D == 10:
            self.u_exact = 0.89060
        elif D == 100:
            self.u_exact = 1.04510
        elif D == 1000:
            self.u_exact = 1.09100
        elif D == 10000:
            self.u_exact = 1.11402
    
    def phi_tf(self, t, X, Y, Z): # M x 1, M x D, M x 1, M x D
        return - Y + Y**3 # M x 1
    
    def g_tf(self, X): # M x D
        return torch.arctan(torch.max(X, 1, keepdim=True)[0])
        # return 1 / (2 + 0.4 * torch.sum(X**2, 1, keepdims=True)) # M x 1
    ...
    def residual(self, t, X):
        ...
        for i in range(self.D):
            ...
            u_xx += d2f_dxidxi
        u = u.reshape(-1)
        residual_pred = u_t.reshape(-1) + (u_xx + u - u**3)
    ...
    def residual_1(self, t, X):
        ...
        u_x = torch.autograd.grad(u.sum(), X, create_graph=True)[0]
        u = u.reshape(-1)
        residual_pred = (u_t.reshape(-1) + (u - u**3)).detach()
        gradient_j = u_t.reshape(-1) + (u - u**3)
...
if __name__ == "__main__":
    ...
    # Training
    model = AllenCahn(Xi, T, M, N, D, layers, device, args.method_PINN, args.batch_size_PINN)
    ...
    df.to_excel(
        "AC_FBSNN_PINN_"+str(args.dim)+"_"+str(args.SEED)+".xlsx",
        index=False
    )